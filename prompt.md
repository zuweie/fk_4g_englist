...
## 修改 /docker/nginx/4ge.conf 文件，这是 nginx 的配置文件。我需要以下配置：
- 监听 8000 端口
- 服务器名字为 4ge.putsoon.com 
- 将请求转发到 3000 端口

## 修改 exercise 页面。
- 我需要另外在 /views/exericse/ 文件夹下添加一个 practice2.ejs 页面。
- 在实现 /views/exercise/pratice2.ejs 页面前，前先阅读 practice.ejs 页面代码。
- practice2.ejs 不再需要使用手写识别功能。所以并不需要加载 blank_cavans.js 和 model.js。
- practice2.ejs 与 practice.ejs 页面风格，元素布局等大概相同。
- practice2.ejs 在每一个 <word-card> 标签内，在 <word-container> 中的每个需要填写的空格，不再需要<blank_canvas>，直接使用单纯的<div>标签，但是保留虚线段的边框。
- 在单词填空下方，设置一个候选字母区，例如 "happy" 中 a,p,p 是需要填写的三个字母，那么我需要随机添加2个字母，例如 e、f，组成候选字母组： e、f、a、p、p，此五个字母随机乱序排放，例如将排列顺序变为：a、e、p、f、p。字母需要使用深绿色。字体大小跟 “happy” 中不用填写的字母，例如 h、y 的字体大小一致。
- 当用户点击要填写字母的空格，将空格高亮为蓝色（虚线边框变成蓝色）。然后当用户点击候选字母组的字母时候，字母会从候选字母组抽走，放置在填写的空格处。
- 如果当填写的空格已经有字母的时候，将字母还原到字母候选区域。
- 当用户将一个字母组的所有的字母都填写完毕 0.5 秒后，判断用户的单词是否正确，若是正确，显示正确状态（如同 practice.ejs 风格一致，单词卡显示绿色），若错误显示错误状态（如同 practice.ejs 风格一致，单词卡显示浅红色）。当用户将任何一个字母退回到字母候选区的时候，字母卡的状态为原始状态，不在为正确或者错误状态。
- 提交按钮的行为和调用的接口与 practice.ejs 完全一致。
- 将原来跳转到 practice.ejs 的路由，改为跳转到 practice2.ejs。

## 修改 practice2.ejs 页面。
- 点击单词的填字框蓝色的高亮不太明显，我需要将高亮状态下蓝色虚线框的虚线**加粗2个像素**。
- 当用户点击了候选框中的字母，字母填入空格后，**下一个空格自动进入高亮状态**。
- **重点**，当用户点击单词的填字框，如果单词填字框中已经有字母，则填字框中的字母退回到候选区，填字框显示空白，填字框的状态为高亮的状态，等待用户填字。
- 单词卡下面的候选字母，靠左排列，与单词卡左对齐。

## 修改 practice2.ejs 页面，第二次
- 用户完成单词卡填写后，单词卡处于正确或者错误状态。当用户点击单词卡上填字框后，字母退回到字母候选区，此时单词卡的状态应当退出正确或者错误状态，还原为初始状态，即没有颜色背景的状态。

## 修改 practice2.ejs 页面，第三次
- 用户单词卡右上角的添加一个喇叭的小按钮，按钮的的链接是 /exercise/playsample?id={单词ID}。

## 修改 practice2.ejs 页面，第四次
- 当用户点击查看答案的按钮 <peek-btn> 的时候弹出一个对话框。
- 对话框的标题为："蠢猪，答对题目才能看答案！"
- 对话框的content是一道数学题，分别是随机，加、减、乘、除 四种题型。
- 加法题目，限定在三位数加两位数。
- 减法题目，限定在三位数减两位数。
- 乘法题目，限定在两位数(<=50)乘两位数(<=50)。
- 除法题目，限定在三位数除两到三位数，被除数大于除数 （a / b， a 需要大于b），除法需要留有两个空格，第一个空格输入商，第二个空格输入余数。
- 输入答案的空格样式与填字空格样式一致。当用点击数学题空格时，弹出一个只有数字的键盘。

## 修改 practice2.ejs 页面，第五次
- 请数学题答案输入框与数学式子放在同一行。
- 当用户点击查看答案的按钮（眼睛按钮）处于打开状态时，直接收起答案即可，不需再次弹出数学题对话框
